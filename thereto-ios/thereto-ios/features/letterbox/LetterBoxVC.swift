...
class LetterBoxVC: BaseVC {
    ...
    static func instance() -> UINavigationController {
        let controller = LetterBoxVC(nibName: nil, bundle: nil).then {
            $0.tabBarItem = UITabBarItem.init(title: "수신함",
    ...
    private func goToLetterDetail(letter: Letter) {
        navigationController?.pushViewController(LetterDetailVC.instance(letter: letter, isSentMode: false), animated: true)
    }
